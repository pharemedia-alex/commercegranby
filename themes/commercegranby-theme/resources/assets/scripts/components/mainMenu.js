...
export default class mainMenu {
  ...
  _hoverMenu(event) {
    ...
    if (event.target.classList.contains('has_child')) {
      ...
      //let menuItemPos = event.target.getBoundingClientRect();
      //let subMenuLeft = menuItemPos.left-20;
      let navWidth =  this._elnav.offsetWidth;
      ...
      if ( _elSubMenu !== null && !_elSubMenu.classList.contains('is-active')) {
        _elSubMenu.style.width = navWidth+'px';
        //_elSubMenu.style.marginLeft = subMenuLeft+'px';
        _elSubMenu.classList.add('is-active');
        this._elSubMenuWrapper.classList.add('is-active');
        this._elHeader.classList.add('hover');
      }
      ...
      if (this._elHeader.classList.contains('hover')) {
          this._elHeader.classList.remove('hover');
      }
    }
  }
